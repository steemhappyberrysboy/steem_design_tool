...
const trFormat = '<tr><td><img src="{{src}}" /></td><td><button src="{{src}}" class="addBtn">Add</button></td></tr>';
...
function click(e) {
  chrome.tabs.executeScript(null,
      // {code:"javascript:function a(){let area=document.getElementsByClassName('smi-gif-picker-textarea')[0];area.blur(); area.value=area.value + '" + e.target.getAttribute('src') + "<br>'; area.focus();}a();"});
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  for (var i = 0; i < addBtns.length; i++) {
    addBtns[i].addEventListener('click', click);
  }
});
